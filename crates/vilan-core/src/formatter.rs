...
/// One comment run re-filled in place — [`reflow_comment_at`]'s answer: the
/// span of the WHOLE run in `source`, and the text that replaces it (each line
/// carrying its own indentation, no trailing newline).
#[derive(Debug, PartialEq, Eq)]
pub struct CommentReflowEdit {
    pub span: Span,
    pub replacement: String,
}

/// E216: re-fill the ONE comment paragraph `offset` sits in, at `width`
/// columns (`[fmt] comment_width`, or the code width when `None`) — the editor's "Reflow this comment" action, which applies E205's
/// filler to that run alone, whatever the package's `[fmt] wrap_comments` says
/// (an explicit action is consent).
///
/// It lives here, beside the printer, because the two questions it has to
/// answer are the printer's own and must not be re-derived in a second place:
/// which consecutive comment lines form ONE paragraph
/// ([`Printer::comment_paragraph_length`]'s rule — a single newline between
/// them, and a blank marker ends the run), and what a re-filled line looks like
/// ([`comment_reflow::reflow`], with its ten never-reflow classes). A run those
/// rules decline, one already filled, and one no line of which exceeds `width`
/// all answer `None`: an action offered where nothing changes is noise.
///
/// The indentation is the run's own, taken from the first line — the caller is
/// editing text in place, not reprinting the file, so the printer's
/// "indentation I am about to emit at" is not available and the written one is
/// the honest answer.
pub fn reflow_comment_at(
    source: &str,
    offset: usize,
    width: Option<usize>,
) -> Option<CommentReflowEdit> {
    // `[fmt] comment_width`, or the code width when the package named none
    // (E215 R1) — the default lives here, where the code width is defined.
    let width = width.unwrap_or(LINE_BUDGET);
    let comments = extract_comments(source);
    let at = comments.iter().position(|(span, _)| {
        let range = span.into_range();
        offset >= range.start && offset <= range.end
    })?;
    // Back to the run's first line, then forward to its last: consecutive
    // comments separated by nothing but ONE newline and whitespace. A blank
    // marker (`//` alone) is a paragraph break the author wrote, so it ends a
    // run on either side.
    let is_blank_marker = |text: &str| {
        comment_reflow::MARKERS
            .iter()
            .any(|marker| text.trim_end() == *marker)
    };
    let contiguous = |before: usize, after: usize| {
        let gap = source
            .get(comments[before].0.into_range().end..comments[after].0.into_range().start)
            .unwrap_or("x");
        gap.chars().all(char::is_whitespace)
            && gap.bytes().filter(|byte| *byte == b'\n').count() == 1
    };
    if is_blank_marker(comments[at].1) {
        return None;
    }
    let mut first = at;
    while first > 0 && !is_blank_marker(comments[first - 1].1) && contiguous(first - 1, first) {
        first -= 1;
    }
    let mut last = at;
    while last + 1 < comments.len()
        && !is_blank_marker(comments[last + 1].1)
        && contiguous(last, last + 1)
    {
        last += 1;
    }
    let lines: Vec<&str> = comments[first..=last]
        .iter()
        .map(|(_, text)| *text)
        .collect();
    // The run's own indentation, and the budget left for the comment on a line
    // that starts with it.
    let start = comments[first].0.into_range().start;
    let line_start = source[..start].rfind('\n').map_or(0, |newline| newline + 1);
    let indent = &source[line_start..start];
    if !indent.chars().all(char::is_whitespace) {
        // A trailing comment after code is never reflowed (E205's eleventh
        // class), and this is what it looks like from here.
        return None;
    }
    let columns: usize = indent
        ...
        .sum();
    let budget = width.checked_sub(columns)?;
    if !lines
        .iter()
        .any(|line| columns + line.chars().count() > width)
    {
        return None;
    }
    let comment_reflow::Reflow::Filled(filled) = comment_reflow::reflow(&lines, budget) else {
        return None;
    };
    let replacement = filled
        ...
        .collect::<Vec<_>>()
        .join("\n");
    let end = comments[last].0.into_range().end;
    let span = Span::from(line_start..end);
    (replacement != source[line_start..end]).then_some(CommentReflowEdit { span, replacement })
}
