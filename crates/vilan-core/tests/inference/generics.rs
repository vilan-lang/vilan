...
// --- B227: an `any` parameter COERCES; it never types an inference hole -----
//
// B13's "adopt a concrete declared parameter" rule let `Type::Any` through its
// guard, so the first call that passed an unfilled closure parameter to an
// `any` parameter WROTE `any` into the shared slot — `print(v)` in the first
// line of a body decided `v` forever, ahead of the enclosing call that
// actually knew. First writer wins, so every later check on that parameter
// was answered by `any` and silenced. `any` is a one-way coercion sink: it
// accepts every type and tells the hole nothing, so it is skipped WITHOUT
// deferring (deferring deadlocks a closure whose only use IS an `any` call).
