...
// --- B318 S3: `only` and the `(impl …)` selectors ----------------------------
//
// `proposal/visibility.md` §2.4–§2.6, §3.2, §3.6, §9 S3. The admission these
// pin is FILE-LEVEL: which of a module's `impl` blocks a file's import list
// admits. The per-importer COLLISION rule (two admitted blocks declaring one
// name) is S4's and is not built here.
//
// The shared exhibit is the paper's probe P7, which is what makes `only` and
// the selectors worth their grammar: `app.vl` never names `x`, imports
// something two levels UNDER it, and `names.md` §4.1's "a child pulls its
// parent" loads `x.vl` — extension impl and all. Today a file's method surface
// is a function of what the whole PROGRAM happened to load.
