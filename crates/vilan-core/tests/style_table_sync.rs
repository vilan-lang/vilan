...
fn a_delegating_condition_is_recorded_on_the_axis_it_delegates_to() {
    ...
    for (name, body) in declared_methods(&source) {
        ...
        // `self.media("48rem", inner)`. A condition whose WHOLE body is one
        // such call must carry the axis of what it delegates to. The primitives
        // have real bodies, so the single-statement test is what separates a
        // delegation from a special case; their own axes are pinned by the
        // order pins in `formatter.rs`.
