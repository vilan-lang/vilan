...
impl Document {
    ...
    fn code_path_completions(
        ...
        let Some(namespace) = self.namespace_in_scope(program, left, colon_offset) else {
            return Vec::new();
        };
        self.namespace_completions(program, namespace)
    }

    /// The struct / enum / module `name` denotes at `offset` — the cursor's
    /// scope out to global: locals, parameters, this file's top-level items, and
    /// everything a `use`/`import` bound.
    ///
    ...
    fn namespace_in_scope(&self, program: &Program, name: &str, offset: usize) -> Option<Id> {
        self.binding_in_scope(program, name, offset)
