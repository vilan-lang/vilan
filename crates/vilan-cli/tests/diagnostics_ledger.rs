...
const RULE_STATEMENT_SITES: usize = 51;
...
const CURATED_RULE_STATEMENTS: &[(&str, &str, &str)] = &[
    ...
    (
        "crates/vilan-core/src/parsing.rs",
        "MODULE_SELF_LEADS_THE_FILE",
        "`mod self;` carries the attributes of the whole file, so it is the file's",
    ),
    (
        "crates/vilan-core/src/parsing.rs",
        "MODULE_SELF_IS_RESERVED",
        "`self` is reserved for the file's own module — `mod self;`, with no body,",
    ),
    (
        "crates/vilan-core/src/parsing.rs",
