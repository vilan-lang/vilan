...
/// (`transport-rpc.md` §9.1, the owner's sketch of 2026-09-07).
///
/// Q9 ruled `&mut self` "the idiomatic in-place receiver" for a service's
/// synchronous session state. The generator does not honour it: the emitted
/// `fun dispatcher(self)` takes `self` by value and every route closes over
/// that copy (`std/src/rpc.vl:3249`, `:3188`, `:3193`), so `&mut self` on an
/// `[rpc]` method fails the build with a diagnostic pointing at the STRUCT —
///
/// ```text
/// in code generated by this attribute: cannot mutate immutable 'self'; declare
/// it `mut self` to mutate this function's copy, or `&mut self` to mutate the
/// caller's value.
/// ```
///
/// — recommending the receiver the author just wrote. `mut self` is the
/// receiver the macro admits, and it compiles: the handler mutates its own
/// copy, returns the new value, and the instance the dispatcher holds never
/// moves. That silent loss is what this pin holds, beside the spelling that
/// DOES work — a `Shared<T>` field, per connection and persistent across calls,
/// which is what A38's factory made possible and what kolt's store uses.
///
/// Held here rather than fixed: §9.1 asks the owner whether the generator
/// should honour `&mut self` or the attribute should refuse it in its own
/// vocabulary. Either ruling changes this test's first two assertions, which is
/// exactly why they are written down.
#[test]
fn a_mut_self_rpc_mutation_is_lost_where_a_shared_field_survives() {
    let dir = temp_project("mut_self");
    write(
        &dir,
        "vilan.toml",
        "[package]\nname = \"app\"\ntarget = \"node\"\n",
...
struct Gate {
	who: str,
	// The sketch's `is_authenticated: bool`, in both spellings.
	plain: bool,
	held: Shared<bool>,
}

impl Gate {
	// `mut self` is the receiver the macro admits (`&mut self` does not
	// build — see this test's doc comment). It mutates the handler's copy.
	[rpc]
	fun login_plain(mut self, password: str): bool {
		if password == "hunter2" {
			self.plain = true;
		}
		self.plain
	...
	// The spelling that persists: the cell is the instance's own.
	[rpc]
	...
	[rpc]
	fun state(self): str {
		i"{self.who}/plain:{self.plain}/held:{self.held.read()}"
	}
}
		...
		Ok(let a) => {
			match GateClient::connect(i"ws://localhost:{port}/", json_codec()) {
				Ok(let b) => {
					...
					print(i"a-login-plain:{a.login_plain("hunter2").unwrap_or(false)}");
...
"#,
    ...
    for expected in [
        ...
        // The handler's own copy sees the write, so the method answers `true` —
        // which is precisely why the loss downstream is silent.
        "a-login-plain:true",
        "a-login-held:true",
        // `plain` did NOT survive the call; `held` did. One connection, one
        // instance, two field kinds.
        "a-after:conn-0/plain:false/held:true",
        // And the second connection's instance is untouched by either — A38's
        // per-connection guarantee, unchanged.
        "b-after:conn-1/plain:false/held:false",
    ] {
        assert!(
            stdout.contains(expected),
            "`{expected}` is missing from the receiver pin's run:\n{stdout}"
        );
    }
    assert!(
        !stdout.contains("a-after:conn-0/plain:true"),
        "`mut self` persisted a plain-field write — the generator now honours an \
         in-place receiver, and `transport-rpc.md` §9.1 (ruling R-A38b) must say \
         so:\n{stdout}"
    );
    assert!(
        !stdout.contains("b-after:conn-1/plain:false/held:true"),
        "the second connection saw the first connection's session state — the \
         factory's per-connection instance is the whole point:\n{stdout}"
    );
    let _ = std::fs::remove_dir_all(&dir);
}
