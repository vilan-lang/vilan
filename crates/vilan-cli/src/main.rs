...
fn report_error_with_labels(
    ...
    let mut labels: Vec<(&str, std::ops::Range<usize>, &str, Color)> = vec![(
        filename,
        primary_span.clone(),
        error.msg.as_str(),
        Color::Red,
    ...
    for (secondary, file) in located {
        let label_filename = file.map(|(name, _)| name).unwrap_or(filename);
        labels.push((
            label_filename,
            secondary.span.into_range(),
    ...
    for (name, text) in &mut files {
        let blank = labels
            .iter()
            .filter(|(label_file, _, _, _)| label_file == name)
            .any(|(_, span, _, _)| snippet(text, span).is_empty() && !text.is_empty());
        if blank {
            text.clear();
        }
    }
    ...
    labels.sort_by_key(|(label_file, span, _, _)| (file_rank(label_file), span.start, span.end));
    ...
    for (label_filename, span, message, color) in labels {
        report = report.with_label(
            Label::new((label_filename.to_string(), span))
                .with_message(message)
                .with_color(color),
        );
    }
