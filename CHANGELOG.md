...
**Found while building it, and pinned rather than fixed here: two functions of one name are not a diagnostic.** Two `fun which(self)` in one `impl` — or across two `impl` blocks on one subject — compile clean, and the *last* one wins. Nothing is said. That is why bindgen emits a type's constructor, its instance members, and its constructor object's statics into a single `impl` sharing one name table: separate blocks would compile and one binding would silently not exist. `crates/vilan-core/tests/bindgen.rs::a_duplicate_function_name_is_silently_shadowed_rather_than_rejected` goes red the day the language rejects it.
