...
3. **`DevChannel.clients` accumulates dead SSE streams** (S; found by the survey)
   STATUS: SHIPPED 2026-08-18 (record: hmr.md appendix)
   The HMR dev channel pushed every SSE connection into
   ...
   **not** reap — a write to a cleanly closed peer succeeds, so a dead
   ...
   bin-only). M2's tier-2 soak remains the long-horizon validation.
