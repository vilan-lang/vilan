...
Status: **ALL FIVE SLICES SHIPPED 2026-08-01 — the arc is complete** (branch `element-syntax`; §10 carries each slice's record, §6 the interactions found en route, and the website exhibit waits on the next release). Prior status: RATIFIED 2026-08-01, backlog H8 filed the same day; before that DRAFT 2026-08-01, open calls all settled same day (Reed took every recommendation); the §9.1 widening probe ran green against the repo compiler. Revised 2026-08-01 after first review (Reed): attributes are paren-form (`placeholder("…")` — the first draft's `=`/hole spelling is gone), chain links in the head wear their dot (`.bind_value(draft)`), self-closing tags space before `/>` by convention, component tags deferred entirely, the `on:` handler call settled. The `view` chain (`guide/ui.md`) is not replaced, deprecated, or changed by this proposal — the sugar lowers to it, and the two forms mix freely in one file, one function, one expression.
...
- **S3 — formatter. SHIPPED 2026-08-01** (branch `element-syntax`) — the canonical element printer replaces S2's verbatim passthrough: inline for at-most-one non-element child within budget, children one per line at +1 with `</tag>` at the element's indent, over-budget heads one item per line with `>`/`/>` at the element's indent (the signature-layout shape), ` />` normalized. Two AST additions the reprint forced, both token-fidelity-driven: `ElementChild::{Hole, Bare}` (a braced hole's `{}` are tokens — `{"x"}` vs `"x"` — so bracedness is structural, never sniffed from source) and `ElementBody.self_closing` (`<div></div>` and `<div />` differ by tokens, so the net FORBIDS converting between them — the ` />` convention is enforced exactly where tokens allow and nowhere else). Comments inside markup relocate below the statement, the chain/list/struct precedent, pinned; the passthrough's silent comment DUPLICATION (slice printed the text while the cursor kept it pending) is gone with the passthrough. Ten `assert_construct` pins in `element_layout` (each asserting token-identity, no-silent-bail, canonical form, idempotence, and round-trip); the corpus program reflowed minimally (the svg split; ` />`), its JS golden proven byte-stable. *Original scope:* print arm + split rules; the ` />` convention; idempotence; the net holds.
- **S4 — docs & editors. SHIPPED 2026-08-01** (branch `element-syntax`) — `spec/grammar.md` (the `element`/`head-item`/`element-name`/`child` productions plus the atom-position and adjacency prose), `spec/lexical.md` §2.4 (the span-adjacent pairs — and the statement that the lexer is untouched), `guide/ui.md` ("Element syntax": both forms side by side, two compile-gated browser fences), `tour/coming-from-javascript.md` (three JSX phrasebook rows), and markup rules in both editor grammars (TextMate + highlight.js: tags, `on:` events, hyphenated attribute names — regex-level, as both grammars are). The two deferred diagnostics landed with the docs on new per-source-text plumbing (`Analyzer.source_texts`, registered at the entry and module seams): the unresolved-`view` import NOTE (fires when the error span's source starts with `<` — only the desugar produces such an accessor span) and the `text(…)`-attribute WARNING (the lowered name argument's span covers the unquoted attribute name, so element origin is detectable and a hand-written `.attr("text", …)` stays silent; rides the `must_use` warnings channel). Three pins. *Original scope:* spec, book page, tour, TextMate, highlight.js; docs compile-gated.
- **S5 — tails. SHIPPED 2026-08-01** (branch `element-syntax`; the arc's last slice) — LSP semantic tokens: a `Tag` kind (rendered as the LSP `type` token) paints opening AND closing tag names, attribute and event names paint as properties, from a raw parse merged before the overlap sweep (the `keyword_hover` per-request pattern). The desugar's scaffolding spans were the real fix: the `attr`/`on`/`child` accessors now take ZERO-WIDTH anchors — the wide spans had painted `<div` as a function and attribute names as methods, and a `.child` link generated for a hole shared the hole expression's exact span, tie-breaking nondeterministically against its contents; the `view` accessor alone keeps `<tag` (the S4 import note reads it) and the LSP suppresses that one token by exact span. `ElementBody` gained `close_tag: Option<Span>` (the parser had the value in hand and dropped it), which also unblocked **linked editing**: `textDocument/linkedEditingRange` is registered, handled (the hover shape: fenced, analyzed-offset in, analyzed-range out), and editing one tag name renames its pair; self-closing elements have no pair. Two LSP pins (markup token classification incl. the non-overlap invariant; linked-pair + no-pair cases). The **exhibit**: `vilan-website` branch `element-syntax-exhibit` (d4ee524) rewrites `art.vl`'s `diagram()` — the six-deep `.child(view(…))` stress case — as markup with the `kw`/`t`/`st` token helpers in holes, DOM-preserving; the whole site compiles and formats with the element-syntax dev toolchain, and the branch ships only after the next vilan release carries the syntax (the deploy rides releases/latest). *Original scope:* semantic tokens; matching-tag niceties; the website page as proof and exhibit. *Post-arc addendum (2026-08-01, after the merge to next):* the concurrent comment-attachment arc (`split-comment-attachment.md`) made S3's relocate-below-statement behavior the odd one out; elements now attach interior comments like the other five splittable constructs — the S3 relocation pin became the attachment pin, plus three new cases (comment forces the split; comment splits the head; trailing comment still relocates, list parity).
