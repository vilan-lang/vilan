...
- **S1 — std groundwork.** Text-node children on both twins; `Slot`/`AttrValue`; widened `child`/`attr` (§9.1, settled). Useful standalone: mixed content becomes expressible in the chain today. Re-bless `ssr-render` goldens and the corpus symbol-rename churn (§9.1). The concrete-generic-impl and emitted-JS probes already ran green (§5, §9.1); the remaining S1 verification is diagnostic quality on the widened signatures.
- **S2 — grammar.** The `Node` element variant, `parse_atom` arm, span-adjacency for `/>` `</` and `on:`, keyword/hyphen attribute names, the desugar pass in the lift slot, the unresolved-`view` note, the str-typed-method-name-as-attribute note (§4). Corpus additions, parser fixtures, recovery.
