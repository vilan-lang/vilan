...
33. **NEW — module-level binding emission order is listing order, not
    dependency order** (M; found 2026-07-22 by WO-1b's probes) — imported
...
10. **NEW — Move the repo to a proper org** (S–M; user request 2026-07-25;
    **prerequisite of D5, public traction**) — the repo lives under the
    ...
    recorded in CI. Also re-check the go-public tripwires (hygiene test,
    private hooks) survive the move, and keep the pseudonym discipline —
    org naming is the user's call.
...
16. **NEW — cross-source diagnostic span renders against the wrong text; CRLF
    ...
    site), not patched by clamping offsets.
...
7. **NEW — Distribution Phase 2** (M; from the release system's recorded plan) — npm
   ...
   (user call 2026-07-18: HMR outranks distribution). **Re-sequenced 2026-07-24
   (user): after F8 Windows support**, with B33 possibly inserted between the
   F7+F5 proposal and its implementation.
