...
29. **Trait conformance is name-only; signatures unchecked — SHIPPED
    ...
    **Residue (b) OPEN** (S) — a `= Self`-defaulted trait generic (`trait
    Add<B = Self>`) interns `B` to the same TypeId as `Self`, so such a
    position is ambiguous and is SKIPPED (`self_ambiguous`, for parameters and
    the return type alike): `impl Meters with Add { fun add(self, b: str) }`
    slips conformance and only errors at use sites. Receiver, arity and
    conventions are still enforced there, and `From<T>`-shape (a
    non-Self-defaulted generic) is unaffected. Recorded fix: compare those
    positions at the declaration-node level, where the AST still distinguishes
    a written `Self` from a written `B`. Pinned `#[ignore]`d as
    `a_self_defaulted_generic_position_with_a_wrong_type_should_be_rejected`.
...
10. **Move the repo to a proper org — SHIPPED** (the owner-string sweep +
    ...
    **The Pages tombstone is LIVE, verified end to end 2026-07-29** (this was
    ...
    the body is.) **Open tail:** (a) `org-migration.md` carries no per-slice
    status markers, so S1–S3 are asserted only in commit prose while S4 is
    provable from the tree — add them so the file stops reading as a plan;
    (b) two personal-identity strings survive OUTSIDE the hygiene gate's reach
    (its needle is `reedsyllas/vilan`, not the bare profile): `AI_STANCE.md`
    links `github.com/ReedSyllas`, and `CODE_OF_CONDUCT.md` publishes a
    personal gmail as the contact. Both are plausibly intended, but the gate
    will never catch a regression there — decide and record. Original entry:
...
21. **NEW — test-suite speed audit** (S–M investigation; user request
...
8. **NEW — Element syntax — ARC COMPLETE 2026-08-01, all five slices
   ...
   (S1): trait-dispatched context requirements union across impls, so the
   browser twin's `child`/`attr` fence even static slots behind `owner_scope`
   (adopted — it matches the documented build-under-a-root model; proposal
   §6); the follow-up is per-instantiation context precision in the coverage
   pass, which the analyzer's monomorphization already knows enough to feed.
