...
40. **NEW — LSP request handlers are not panic-fenced** (S–M; recorded
    2026-07-28 after the const-preview `truncate` panic) — `Document::analyze`
    runs under `catch_unwind` (a malformed input degrades gracefully), but a
    panic in any REQUEST HANDLER unwinds through the async runtime and aborts
    the whole server: exit 101, and after five crashes in three minutes the
    client stops restarting it — one bad hover locked the user out of every
    LSP feature. The `truncate` root cause is fixed and pinned; this records
    the blast-radius question. Design questions before building: what a
    fenced handler answers (internal error vs the request's empty default),
    where the panic is logged so it is not silently swallowed, and whether
    any state can be left poisoned mid-handler (the document map is only
    read by handlers, which suggests no). Shape: a `catch_unwind` seam at
    the handler boundary, applied uniformly rather than per-handler.
...
10. **NEW — Third-party notices for the distributed binaries** (S; licensing
    review find, 2026-07-28) — the release archives, npm platform packages,
    and brew install carry vilan's two licenses but no notices for the ~130
    statically-linked MIT/ISC/Unicode-3.0 crates, whose licenses ask that
    their notices accompany copies. Ecosystem practice is mixed; the strict
    reading ships a generated `THIRD-PARTY-NOTICES.txt`. Generate with
    `cargo-about` or `cargo-bundle-licenses` in release.yml's build legs and
    add it to the archive copy list beside LICENSE-MIT/LICENSE-APACHE
    (npm-package.sh inherits it with one `cp`). The vsix's equivalent shipped
    2026-07-28 (`editors/vscode/ThirdPartyNotices.txt`).
