...
7. **Distribution Phase 2 — BUILT 2026-07-25, all channels (F7 S1-S3;
   ...
   **PROVISIONED 2026-07-29** — and three of the four credentials are not
   ...
   - **npm** — was `NPM_TOKEN`, now trusted publishing (OIDC). The token was
     ...
     longer reads a secret. **Not yet proven** — the six per-package trusted
     publishers are a pending user step on npmjs.com and must land before the
     next tag, and `NPM_TOKEN` is revoked only after a release publishes green
     by OIDC. `distribution.md` §2 + §7 carry the mechanics and the checklist.
