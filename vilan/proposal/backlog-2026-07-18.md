...
8. **NEW — Element syntax** (L; proposal `element-syntax.md` RATIFIED
   ...
   → S2 grammar + desugar
   → S3 formatter (KNOWN_FORMATTER_BAILS must not grow) → S4
   spec/book/TextMate/highlight.js → S5 LSP tail + a website page as the
   exhibit. Deferred entirely: component tags, fragments, bare text, the `=`
   attr spelling.
