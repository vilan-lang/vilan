...
7. **`vilan/examples` cleanup; single-package becomes the default shape —
   ...
   D6's recorded "rides D7" item and stays there. (b) **NEW, see E22** — seven
   of the nine examples have no build gate. Original entry: (S–M; user request
...
7. **Distribution Phase 2 — BUILT 2026-07-25, all channels (F7 S1-S3;
   ...
   **PROVISIONED 2026-07-29** — and three of the four credentials are not
   ...
   underneath it:
   - **npm** — `NPM_TOKEN`, set. A BRIDGE: npm is retiring 2FA-bypass tokens
     (account changes Aug 2026, direct publishing ~Jan 2027). The destination
     is trusted publishing (OIDC), which cannot perform a package's FIRST
     publish, so the token has to create the six packages before the migration
     is even possible. Queued.
   - **VS Code Marketplace** — `AZURE_CLIENT_ID` / `AZURE_TENANT_ID`, NOT
     ...
     repo's `marketplace` environment — no stored credential.
