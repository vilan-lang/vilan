...
43. **NEW — a statement's split does not descend into a call's argument, and
    `push(T { … })` is the shape that wants it** (S–M design, S implementation;
    recorded 2026-08-01 alongside 42) — `Split::Statement` reaches the
...
8. **NEW — Element syntax** (L; proposal `element-syntax.md` RATIFIED
   2026-08-01) — HTML-flavored sugar lowering to the `view` chain in the
   ...
   → S3 formatter (canonical element layout; the bail gate asserts an EMPTY
   set — no ledger exists) → S4 spec/book/TextMate/highlight.js + the
   unresolved-`view` import note and str-typed-method-name-as-attribute note
   (need per-source text in the analyzer) → S5 LSP tail + a website page as
   the exhibit. Deferred entirely: component tags, fragments, bare text, the
   `=` attr spelling.
