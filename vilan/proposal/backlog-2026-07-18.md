...
39. **the LSP's request path is doing too much work — leg (a) SHIPPED
    ...
    non-file URL sweeps as before. **Legs (b) and (c) remain open** —
    (b) semantic-token delta/range (`result_id: None` still forecloses
    it), (c) incremental sync (still `FULL`; the inlay-hint viewport
    filter stays approximate until it lands). Original entry: (M; recorded
