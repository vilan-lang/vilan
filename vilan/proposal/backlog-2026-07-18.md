...
25. **NEW — run the suite's binaries in parallel** (M; E21's largest lever,
    `suite-speed.md` §2) — 130.7 s of serial execution vs a 29.9 s longest
    binary is a theoretical ~4.4×; cargo-nextest is the candidate
    instrument (per-test processes make today's in-process serialization
    unnecessary rather than broken; port-binding legs already survived
    E19's port-0 migration). Clear per-binary: stdout-parsing e2e legs,
    node-spawn storms under load (the E20 flake history), CI parity.
    E26's measurement (`suite-speed.md` §2.1) revises the ceiling: it is
    total CPU ÷ 16, not the longest binary — inference already saturates
    the machine while it runs, so overlapping other binaries with it buys
    little during those seconds. Re-estimate from summed per-binary user
    time before promising ~4.4×.
...
27. **NEW — parallelize the docs gate and interpreter cases** (S; E21) —
    both are serial loops over independent compiles; corpus.rs's 8-way
    `thread::scope` chunks are the proven in-suite shape. Est: 16.7 s →
    ~4 s and 14.9 s → ~4 s.

28. **NEW — share the LSP's analyzed test fixtures** (S–M; E21; DECIDE
    AFTER E25) — the unit binary's 29.9 s is ~81 `Document::analyze`
    fixture sites at ~150 ms each; a `OnceLock`-shared analysis per
    distinct source pays each once. Under nextest's per-test processes
    this lever mostly evaporates — the runner decision comes first. The
    Linux leak harness hides inside this binary; measure it separately
    before touching.

29. **NEW — cut the 16 s relink tax** (S–M; E21) — after any vilan-core
    edit, 43+ test binaries relink at ~490 % of 1600 % CPU. Sub-levers:
    a faster linker (neither mold nor lld is installed; either typically
    halves link-heavy rebuilds) and consolidating same-subject test files
    (the parse_* five, the hmr trio) to cut link jobs — weighed against
    per-binary isolation, and worth less if E25 lands via nextest.
    Evidence first: `cargo build --timings` on the relink.

30. **NEW — inference.rs is ~276 CPU-seconds of repeated std analysis**
    (M; filed 2026-08-02 by E26's measurement, `suite-speed.md` §2.1) — a
    single ~10-line `assert_compiles` case costs ~170 ms of single-threaded
    pipeline, so the binary's ~1400 `compile()` calls burn the CPU that IS
    its 19 s wall; the program under test is ten lines, so the cost must be
    dominated by re-resolving and re-analyzing std per call. Lever: share
    the analyzed std across the harness's compiles — every case still runs
    its own program through the real pipeline; only the std prefix is
    shared. Same shape as E28 and the same nextest interaction: in-process
    sharing evaporates under per-test processes; only a disk-cached std
    snapshot survives both, and that is an analyzer arc, not a test tweak.
    Profile first (how much of the 170 ms IS std), decide with E25/E28.
...
8. **NEW — Element syntax — ARC COMPLETE 2026-08-01, all five slices
   ...
   refactor); threading stays union-based. Five pins, two proven red first.
