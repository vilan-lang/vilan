...
39. **the LSP's request path is doing too much work — leg (a) SHIPPED
    ...
    response by design). **Leg (c) remains open** — incremental sync (still
    `FULL`; the inlay-hint viewport filter stays approximate until it
    lands). Original entry: (M; recorded
