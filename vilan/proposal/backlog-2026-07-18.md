...
25. **run the suite's binaries in parallel — SHIPPED 2026-08-02**
    ...
    gate; release.yml's tag-time gate unchanged. Recorded cost: user CPU
...
28. **NEW — share the LSP's analyzed test fixtures** (S–M; E21; RESHAPED
    BY E25 2026-08-02) — the unit binary's 29.9 s is ~81
    `Document::analyze` fixture sites at ~150 ms each. E25 landed via
    nextest, so the filed `OnceLock` shape is foreclosed (per-test
    processes share nothing in-process): the lever now means a disk-cached
    analysis snapshot (an analyzer arc) or cutting per-analysis cost
    itself — and the payoff shrank, since the leak plateaus and the CPU
    floor, not fixture repetition, now bound the suite wall. Start with a
    user-time check per the E26 lesson. The Linux leak harness hides
    inside this binary; measure it separately before touching.

29. **NEW — cut the 16 s relink tax** (S–M; E21; RESHAPED BY E25
    2026-08-02) — after any vilan-core edit, 43+ test binaries relink at
    ~490 % of 1600 % CPU. E25 landed via nextest, which kills the
    consolidation sub-lever (nextest prefers many binaries) and raises
    the stakes on the other: 16 s of relink is now a QUARTER of a 63.5 s
    suite run. Remaining lever: a faster linker (neither mold nor lld is
    installed; either typically halves link-heavy rebuilds).
...
30. **NEW — inference.rs is ~276 CPU-seconds of repeated std analysis**
    ...
    shared. Same shape as E28, and E25's outcome (nextest, 2026-08-02)
    settles the interaction: in-process sharing is foreclosed by per-test
    processes; only a disk-cached std snapshot survives, and that is an
    analyzer arc, not a test tweak. Profile first (how much of the 170 ms
    IS std), weigh against E28 as one arc if taken up.
