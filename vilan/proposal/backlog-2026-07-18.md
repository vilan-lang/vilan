...
21. **test-suite speed audit — AUDIT DONE 2026-08-03**
    (`proposal/suite-speed.md` is the record: the measured profile and the
    slice list, filed here as E25–E29) — the headline numbers: a warm-tree
    ...
    ~81 real `Document::analyze` fixtures, inference (18.7 s) is 534 node
    spawns at ~35 ms of startup each, docs (16.7 s) and interpreter
    (14.9 s) are serial loops over independent compiles, and corpus is
    already 8-way parallel — the shape to copy. Recommended sequence:
    E26 → E27 → E25 → (E28/E29 per E25's outcome). Original entry: (S–M investigation; user request
...
25. **NEW — run the suite's binaries in parallel** (M; E21's largest lever,
    ...
    Sequenced AFTER E26 (whose amortization per-test processes would
    foreclose). Est: 131 s → ~35–45 s.

26. **NEW — batch inference's node runs** (S–M; E21's cleanest lever) —
    534 `assert_compiles_and_runs` spawns × ~35 ms of node startup IS
    inference.rs's 18.7 s. Execute the emitted programs through one (or a
    few) node processes with per-program output markers; every assertion
    stays byte-identical. Est: 18.7 s → ~3–4 s. Land before E25.
