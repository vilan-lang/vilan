...
15. **`run` in a multi-node workspace — SHIPPED 2026-07-22, flag-only v1**
    ...
    and drive no restart; kolt-shape e2e pinned). Recorded follow-up: a
    manifest-designated default entry so kolt needs no flag.
...
16. **NEW — cross-source diagnostic span renders against the wrong text; CRLF
    makes it fatal** (S–M; found 2026-07-24 by the windows-arc S2 adversarial
...
17. **NEW — LSP drops the note on module-attributed diagnostics** (S; found
...
20. **the watch-round sidecar flake — MITIGATED 2026-07-25 after a third
    strike** (struck 20.25s local 2026-07-24, 20.5s ubuntu leg, 20.97s
    windows leg — each a bare overshoot of the 20s deadline on a round that
    then completed, flipping required CI legs red on identical-code runs) —
    the deadline was raised to 120s (a deadline catches HANGS, not a loaded
    scheduler; all three strikes were finishing-late, not hung). REMAINING
    (the real fix, take on demand): inject the change event instead of
    racing the mtime poller. Original entry:
...
19. **NEW — the `free_port` TOCTOU flake, three strikes** (S–M; filed
