...
80. **NEW — the requirement trace stops at the CLI and the editor: HMR overlay and the wasm playground surface message + note only** (S; found by E78's lane 2026-08-20)
    STATUS: OPEN
    E78's `trace: Vec<Note>` channel is rendered by the CLI (multi-label
    ariadne) and the LSP (`DiagnosticRelatedInformation`), but the HMR
    overlay and the wasm playground's diagnostics pane still surface
    message + C3 note only — a playground user pasting the owner's E78
    example sees the anchor without the chain. Thread the trace through
    both surfaces (the wasm boundary's diagnostic shape and the overlay's
    renderer). Adjacent, the owner's call, parked here: whether
    diagnostics-standard.md gains a section naming the trace channel (C3
    stays one-note; the trace is a new, separate contract). Record:
    E78's report; ledger rows 241–243 are the label heads.
