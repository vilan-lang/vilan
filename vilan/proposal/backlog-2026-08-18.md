...
77. **NEW — `head()`/`body()` on a SUPPLIED shell are silently inert** (S; found by E70's lane 2026-08-20)
    STATUS: OPEN (RULED 2026-08-20: COMPOSE, as recommended — the hatches splice into the supplied shell and the composed page runs the check_shell rules, uniform with E70's generated arm; lane e77-supplied-hatches, Order 8)
...
78. **NEW — the unprovided-context error underlines the whole call chain, not just its anchor** (M; owner's 2026-08-20 ask)
    ...
    `owner_scope` coverage flavor (the same walk serves both).
    **Owner clarification, 2026-08-20: only the UNCOVERED upstream calls
    underline** — a call that provides the context is clean and stops
    the trace. The owner's example, which becomes the item's acceptance
    shape (and a pin):
    ...
    `b`'s `a()` and `main`'s `b()` carry spans; `c`'s covered call
    carries nothing and `c` itself is clean. This is exactly the edge
    filter E74's walk already applies (it descends through UNBOUND
    callers only — a covered call is never blamed), so the clarification
    confirms the walk rather than changing it; the item's work is
    keeping the traversed path and rendering it. Record: E74's machinery
    (context.rs, async-polymorphism.md §A.4b); the ledger rows 222/223.
...
11. **Playground wasm retention policy** (M)
    STATUS: OPEN (RULED 2026-08-20: prune to the newest ~6 version directories, as recommended — git history keeps the rest, K7's no-pin ruling makes old versions a non-promise; lane k11-wasm-retention, Order 8)
