...
11. **NEW — `cut-release.sh` accepts a family marker with no entry** (S; found 2026-08-20 during the E81 records check)
    A `<!-- family: tooling -->` line followed by blank lines and a `---`
    — union debris from a CHANGELOG merge — parsed as nothing: the marker
    set `pending_family`, the rule cleared it at the boundary, and no
    head ever consumed it, so the dry-run stayed green over a section
    carrying an orphan that would have ridden into the release section
    as a dangling comment. Found by an anchored parity count (`^<!--
    family:` against `^\*\*` heads: 35 to 33 — one prose mention of the
    marker, one orphan). Make the parser REFUSE a marker that reaches
    the next marker, rule, or section end without a head, and pin it
    beside the existing refusals — §7.2 step 3's "never guesses" already
    covers the spirit. Until then: run the anchored parity count after
    every CHANGELOG union (the lesson is in memory too).
