...
involved. The guides are the reference — [Reactive state](../../docs/guide/reactive.md),
[Building UI](../../docs/guide/ui.md), [Styling](../../docs/guide/styling.md) —
and this is the working code they describe.
...
- **`bind_each`** — a keyed reactive list: rows move with their keys, only a
  changed row re-renders, a removed row is disposed.
...
browsers restrict ES modules over `file://`.) The emitted `app.js`/`app.css` are
