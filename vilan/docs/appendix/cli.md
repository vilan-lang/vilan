...
- `--entry <name>` — in a package (or workspace) with more than one
  ...
  just don't run. The manifest's `default-entry` makes the flag
...
`--check` reports the files that would change and exits 1 if any —
the CI spelling. Nothing is rewritten.
